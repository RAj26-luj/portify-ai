...
export default function EducationPage() {
  ...
  const load = async (pId?: string) => {
    try {
      ...
      if (!result || !result.success || !("data" in result) || !Array.isArray(result.data)) {
        throw new Error(
          "error" in result ? result.error : "Failed compiling target academic traces from servers."
        );
      }
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(
          "error" in result
            ? result.error
            : "Downstream mutation dropped structural transaction frames."
        );
      }
      ...
      setActionError(
        err instanceof Error
          ? err.message
          : "Failed to safely complete index record delete sequence."
      );
    } finally {
  ...
  const filteredEducations = educations
    .filter((edu) => {
      const matchCriteria =
        `${edu.institution} ${edu.degree} ${edu.fieldOfStudy || ""} ${edu.location || ""}`.toLowerCase();
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[45vh] sm:min-h-[55vh] gap-4 bg-[#050505] text-zinc-500 border border-zinc-900 rounded-xl select-none font-mono p-4 text-center">
        ...
        <p className="text-[10px] sm:text-xs uppercase tracking-widest">
          // Re-indexing educational tracking timeline...
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-900 pb-4 sm:pb-5">
        <div className="space-y-1">
          ...
          <p className="text-[11px] sm:text-xs text-zinc-500 font-medium leading-relaxed max-w-2xl">
            Manage your corporate training backgrounds, university degrees, secondary schools, and
      ...
      {filteredEducations.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-800 bg-zinc-950/10 p-6 sm:p-12 text-center max-w-xl mx-auto my-4 animate-fadeIn w-full">
          ...
          <h3 className="text-xs sm:text-sm font-bold text-zinc-200 tracking-tight">
            {educations.length === 0
              ? "Academic History Context Empty"
                  ...
                  {activeConfirmDeleteId === edu.id ? (
                    ...
                    <div className="flex items-center justify-between gap-3 w-full">
                      <span className="text-[9px] font-mono text-zinc-600">
                        {edu.startDate ? new Date(edu.startDate).getFullYear() : "Init"} —{" "}
                        {edu.currentlyStudying
                          ? "Present"
                          : edu.endDate
                ...
                {filteredEducations.map((edu) => (
                  <div key={edu.id} className="relative group">
                    <EducationCard
                      ...
                      endDate={
                        edu.endDate ? new Date(edu.endDate).toISOString().split("T")[0] : undefined
                      }
                      currentlyStudying={edu.currentlyStudying}
                        ...
                        <div className="flex items-start gap-3 text-red-400">
                          ...
                          <div className="space-y-1">
                            <h4 className="text-xs font-mono font-bold uppercase tracking-wider">
                              Purge Academic Record?
                            </h4>
                            <p className="text-[11px] text-zinc-500 leading-normal">
                              This structural change will immediately decouple this historical
                              credential profile trace from your metrics dashboard layer.
                            </p>
                          </div>
                        </div>
                        ...
                        <td className="py-3.5 px-4 text-zinc-400 font-mono text-[11px]">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>
                              ...
                              {" — "}
                              {edu.currentlyStudying ? (
                                <span className="text-blue-400 font-semibold text-[10px] uppercase bg-blue-500/5 border border-blue-500/10 px-1 rounded">
                                  Active
                                </span>
                              ) : edu.endDate ? (
