...
export default function ProjectsPage() {
        ...
        if (!portfolioResult || !portfolioResult.success || !portfolioResult.data) {
          throw new Error(
            "error" in portfolioResult && typeof portfolioResult.error === "string"
              ...
              : "Portfolio tracking context was not resolved."
          );
        }
      ...
      if (!result || !result.success || !("data" in result) || !Array.isArray(result.data)) {
        throw new Error(
          "error" in result && typeof result.error === "string"
            ? result.error
            : "Failed compiling your project arrays."
        );
      }
  ...
  async function handleDelete(id: string, bypassPrompt = false) {
    ...
    if (!isMobile) {
      const confirmDelete = window.confirm(
        "Are you sure you want to remove this project showcase permanently? All nested tracking sub-metrics will be dropped."
      );
    ...
    try {
      ...
      setActionError(
        error instanceof Error
          ? error.message
          : "Unable to safely purge selected project index node. Please try again."
      );
    } finally {
  ...
  const filteredProjects = projects
    .filter((project) => {
      const matchCriteria =
        `${project.title} ${project.shortDescription || ""} ${project.category || ""} ${project.techStack?.join(" ") || ""}`.toLowerCase();
  ...
  const isShowcaseIncomplete =
    projects.length > 0 && projects.some((p) => !p.githubUrl || !p.liveUrl || !p.description);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-900 pb-4 sm:pb-5 px-4 sm:px-0">
        <div className="space-y-1">
          ...
          <p className="text-[11px] sm:text-xs text-zinc-500 font-medium leading-relaxed max-w-2xl">
            Architect, configure, and manage personal, academic, or professional projects
            highlighted across public template canvases.
          </p>
        </div>
        ...
        <div className="rounded-none sm:rounded-xl border-y sm:border border-blue-500/10 bg-gradient-to-r from-blue-500/[0.03] to-transparent p-4 flex gap-2.5 items-start animate-fadeIn w-full mx-4 sm:mx-0">
          ...
          <div className="space-y-0.5 flex-1 min-w-0">
            ...
            <p className="text-[11px] sm:text-xs text-zinc-500 leading-relaxed">
              We highly recommend filling all fields entirely. Adding clean code repositories,
              active endpoints, and full-length problem statements provides deep empirical insight
              that helps recruiters review capabilities contextually.
            </p>
          </div>
        </div>
      )}
      ...
      {filteredProjects.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-none sm:rounded-xl border-y sm:border border-dashed border-zinc-800 bg-zinc-950/10 p-8 sm:p-12 text-center max-w-xl mx-auto my-4 animate-fadeIn w-full">
          ...
          <h3 className="text-xs sm:text-sm font-bold text-zinc-200 tracking-tight">
            {projects.length === 0
              ? "Projects Portfolio Matrix Unpopulated"
                  ...
                  {mobileConfirmDeleteId === project.id ? (
                    ...
                    <div className="flex items-center justify-between gap-3 w-full">
                      <span className="text-[9px] font-mono text-zinc-600">
                        {project.startDate ? new Date(project.startDate).getFullYear() : "Init"} —{" "}
                        {project.endDate ? new Date(project.endDate).getFullYear() : "Active"}
                      </span>
                    ...
                    {filteredProjects.map((project) => (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4 text-zinc-400 font-medium capitalize">
                          {project.category ? (
                            project.category.replace("_", " ").toLowerCase()
                          ) : (
                            <span className="text-zinc-700 italic">Standard</span>
                          )}
                        </td>
