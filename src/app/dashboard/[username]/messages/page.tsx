...
import { getContactMessages, markContactMessageSeen } from "@/actions/contact-message";
      ...
      if (!result || !result.success || !("data" in result) || !Array.isArray(result.data)) {
        throw new Error(
          "error" in result && typeof result.error === "string"
            ? result.error
            : "Failed compiling your inbox logs."
        );
      }
    ...
    try {
      ...
      if (!result || !result.success) {
        throw new Error(
          "error" in result && typeof result.error === "string"
            ? result.error
            : "Failed updating visibility markers."
        );
      }
      ...
      alert(
        err instanceof Error ? err.message : "Failed to mark message as seen. Please try again."
      );
    } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[45vh] sm:min-h-[55vh] gap-4 bg-[#050505] text-zinc-500 border border-zinc-900 rounded-xl select-none font-mono p-4 text-center">
        ...
        <p className="text-[10px] sm:text-xs uppercase tracking-widest">
          // Synchronizing dashboard message desk...
        </p>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-xl my-6 sm:my-12 rounded-xl border border-red-500/10 bg-gradient-to-b from-red-500/5 to-transparent p-4 sm:p-5 shadow-2xl flex gap-3 items-start text-white font-sans w-full">
        ...
        <div className="space-y-2 flex-1 min-w-0">
          ...
          <button
            ...
            className="w-full inline-flex h-8 items-center justify-center rounded-md border border-zinc-800 bg-zinc-900 text-zinc-300 transition-colors hover:bg-zinc-800 hover:text-white"
          >
    ...
    <div className="space-y-4 sm:space-y-6 text-white max-w-[1440px] mx-auto font-sans antialiased px-3 sm:px-6 lg:px-8 py-4 sm:py-6 overflow-x-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-900 pb-4 sm:pb-5">
        <div className="space-y-1">
          ...
          <p className="text-[11px] sm:text-xs text-zinc-500 font-medium leading-relaxed">
            Monitor, track, and parse direct contact requests submitted contextually by public
            portfolio audience members.
          </p>
        </div>
      ...
      <div className="text-[9px] sm:text-[11px] text-amber-400 font-mono bg-amber-500/5 border border-amber-500/10 rounded-lg p-3 sm:p-4 leading-relaxed flex items-start gap-2 sm:gap-2.5 animate-fadeIn">
        ...
        <span className="space-y-0.5">
          <strong className="text-zinc-200 uppercase tracking-wide block text-[10px] sm:text-xs mb-0.5">
            System Protocol // External Communication Routing
          </strong>
          All responses happen completely outside of Portify AI. Click{" "}
          <span className="text-zinc-100 font-bold underline">Mail Them</span> to trigger a safe
          window redirect route that automatically spawns a browser-based compose tab populated with
          the selected lead credentials.
        </span>
      </div>
              ...
              >
                ...
                <div className="space-y-1 pr-10">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-100">
                    <User
                      size={11}
                      className={!message.isSeen ? "text-blue-400" : "text-zinc-600"}
                    />
                    <span className="truncate max-w-[180px]">{message.visitorName}</span>
                  </div>
                  <p className="text-[10px] font-mono text-zinc-500 truncate select-all">
                ...
                <div
                  className="flex items-center justify-between gap-3 pt-2.5 border-t border-zinc-900"
                      ...
                      >
                        <td className="py-3.5 px-4 min-w-[150px]">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <div
                              className={`font-bold transition-colors truncate max-w-[140px] ${!message.isSeen ? "text-white group-hover/row:text-blue-400" : "text-zinc-400 group-hover/row:text-white"}`}
                            >
                              {message.visitorName}
                            </div>
                          </div>
                        ...
                        <td className="py-3.5 px-4 text-zinc-400 max-w-xs truncate font-medium italic">
                          {message.note ? (
                            ...
                            <span className="text-zinc-700 not-italic">
                              No custom abstract log attached
                            </span>
                          )}
                        </td>
                        ...
                        >
                          <div className="flex items-center justify-end gap-2.5">
                            ...
                            >
                              ...
                              <ExternalLink
                                size={10}
                                className="text-zinc-500 group-hover/row:text-zinc-300"
                              />
                            </button>
                            {!message.isSeen && (
                              <button
