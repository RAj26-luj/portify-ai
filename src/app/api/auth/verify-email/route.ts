    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "The account validation ledger is temporarily locked processing secure credentials. Please submit your token string again.",
      },
      { status: 503 }
    );
  }
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      return handleVerifyOtpRouteError(
        jsonError,
        "Failed to decode incoming authentication data stream blocks.",
        400
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Validation failed: A target email address and matching 6-digit OTP code string are required.",
        },
        { status: 400 }
      );
    }
    ...
    if (!verification || verification.expires < new Date()) {
      return NextResponse.json(
        {
          success: false,
          error: "The activation code provided is incorrect, completely invalid, or has expired.",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return handleVerifyOtpRouteError(
        dbError,
        "Failed to map existing profile reference data lines."
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Verification aborted: No matching user account context exists for this credential.",
        },
        { status: 404 }
      );
    }
    ...
    try {
      ...
      return handleVerifyOtpRouteError(
        txError,
        "Failed to commit unified account confirmation changes onto database tables."
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message:
        "Email verified successfully. Your profile has been queued for administrative approval review.",
    });
  } catch (globalCatchError) {
    return handleVerifyOtpRouteError(
      globalCatchError,
      "The OTP check pipeline encountered an unhandled execution core thread break."
    );
  }
}
