...
function handleResumeImportRouteError(error: any, fallbackMessage: string, status = 500) {
  ...
  if (errorMessage.includes("SyntaxError") || errorMessage.includes("JSON")) {
    return NextResponse.json(
      {
        success: false,
        error: "Malformed request payload layout configuration constraints format.",
      },
      { status: 400 }
    );
  }
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "The resume schema ingestion pipeline is performing routine cluster modifications. Please import again.",
      },
      { status: 503 }
    );
  }
...
export async function POST(req: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Authentication validation missing: Active user identification signature is required.",
        },
        { status: 401 }
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Too many consecutive file parsing updates sent from your connection. Please retry in a minute.",
        },
        { status: 429 }
      );
    }
    ...
    try {
      ...
      return handleResumeImportRouteError(
        jsonError,
        "Failed decoding streaming payload structural parameters format layout rules.",
        400
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Validation failed: Missing structural portfolio tracking target index or raw data text parameter metrics.",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return handleResumeImportRouteError(
        lookupError,
        "The optimization engine failed to map your active profile structural configurations."
      );
    }
    ...
    try {
      ...
      return handleResumeImportRouteError(
        actionError,
        "The intelligence parsing model failed converting unformatted layout blocks back to schema structures."
      );
    }
    ...
    return handleResumeImportRouteError(
      globalCatchError,
      "The file upload mapping gateway encountered an unhandled execution core thread break."
    );
  }
}
