...
import { Loader2, Terminal, Calendar, Briefcase, MapPin, Globe, Plus, X, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function ExperienceForm({
  ...
  // Track global form data deltas to toggle action properties safely
  useEffect(() => {
    ...
    const areResponsibilitiesChanged = JSON.stringify(responsibilities) !== JSON.stringify(initialData?.responsibilities ?? []);
    const areTechsChanged = JSON.stringify(technologies) !== JSON.stringify(initialData?.technologies ?? []);

    setHasChanges(
      isCompanyChanged || isPositionChanged || isTypeChanged || isLocationChanged ||
      isWebsiteChanged || isLogoChanged || isBannerChanged || isDescChanged ||
      isStartChanged || isEndChanged || isCurrentChanged || areResponsibilitiesChanged || areTechsChanged
    );
  }, [company, position, employmentType, location, companyWebsite, companyLogo, companyBanner, responsibilities, technologies, startDate, endDate, currentlyWorking, description, initialData]);
  ...
  const isEndDateInvalid = !!(!currentlyWorking && startDate && endDate && new Date(endDate) < new Date(startDate));

  const isFormInvalid = isCompanyInvalid || isPositionInvalid || isEmploymentTypeInvalid || isStartDateInvalid || isEndDateInvalid;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-3 sm:p-4 select-none animate-fadeIn">
      ...
      >
        ...
        <div className="flex justify-between items-center border-b border-white/5 pb-2.5 sm:pb-3">
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-zinc-500 mt-0.5 font-medium leading-tight font-mono uppercase tracking-wider">
              ...
              {hasChanges && (
                <span className="ml-2 text-[8px] font-mono text-blue-400 bg-blue-500/5 border border-blue-500/10 px-1.5 py-0.5 rounded uppercase font-bold tracking-normal">edited_deltas</span>
              )}
            </p>
          </div>
        ...
        <div className="bg-[#121214] border border-white/5 rounded-lg p-3 text-[11px] text-zinc-400 leading-relaxed font-sans">
          <strong className="text-zinc-200">Employment Record Track Node:</strong> Mount an active industrial career block element below. Explicitly populating responsibility metrics and technology stack labels enhances portfolio matrix discoverability graphs.
        </div>

        {/* SECTION 1: BASIC INFORMATION */}
        <div className="space-y-3.5 sm:space-y-4 border border-white/5 p-3 sm:p-4 rounded-xl bg-[#070708]">
          <h3 className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Basic Information Sector</h3>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Company Entity <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
              <div className="flex items-center gap-1">
                {isTouched && isCompanyInvalid ? (
                  <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase flex items-center gap-0.5"><AlertCircle size={9} /> missing_entity</span>
                ) : company.trim() ? (
                  <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5"><CheckCircle2 size={9} /> tracked</span>
                ) : null}
            ...
            />
            {isTouched && isCompanyInvalid && (
              <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Field missing: Corporate body namespace parameter must be populated.</p>
            )}
          </div>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Position Title <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
              <div className="flex items-center gap-1">
                {isTouched && isPositionInvalid ? (
                  ...
                  <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5"><CheckCircle2 size={9} /> confirmed</span>
                ) : null}
              </div>
            </label>
            <input
            ...
            />
            {isTouched && isPositionInvalid && (
              <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Field missing: Specific position title parameter must be established.</p>
            )}
          </div>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Employment Type <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
              <div className="flex items-center gap-1">
                {isTouched && isEmploymentTypeInvalid ? (
                  <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase flex items-center gap-0.5"><AlertCircle size={9} /> missing_type</span>
                ) : employmentType ? (
                  <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5"><CheckCircle2 size={9} /> mapped</span>
                ) : null}
              </div>
            </label>
            <select
            ...
            >
              <option value="" className="bg-[#0C0C0E]">Select Employment Vector</option>
              <option value="FULL_TIME" className="bg-[#0C0C0E]">Full Time</option>
              <option value="PART_TIME" className="bg-[#0C0C0E]">Part Time</option>
              <option value="INTERNSHIP" className="bg-[#0C0C0E]">Internship</option>
              <option value="FREELANCE" className="bg-[#0C0C0E]">Freelance</option>
              <option value="CONTRACT" className="bg-[#0C0C0E]">Contract</option>
            </select>
          </div>
        </div>

        {/* SECTION 2: COMPANY DETAILS & IMAGE HANDLING */}
        <div className="space-y-3.5 sm:space-y-4 border border-white/5 p-3 sm:p-4 rounded-xl bg-[#070708]">
          <h3 className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Corporate Environmental Context</h3>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Geographic / Remote Location <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
          ...
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Company Website URL <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
          ...
          {/* ASSET ACQUISITION ADVICE HINT */}
          <div className={scrapeRecommendationStyle}>
            <Globe size={11} className="shrink-0 mt-0.5 text-amber-400" />
            <span>
              <strong>Brand Image Scraping Advice:</strong> For corporate logo indicators or branch cover art layers, we recommend performing a Google Image search for the emblem, right-clicking to click <strong>&quot;Copy Image Address / URL&quot;</strong>, and passing the raw string target here if local media streams are missing.
            </span>
          </div>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Company Logo Insignia <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
            </label>
            <input
              type="file"
              accept="image/*"
          ...
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Office / Team Background Cover Banner <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
            </label>
            <input
              type="file"
              accept="image/*"
            ...
            {companyBanner && (
              <div className="mt-1.5 p-1 bg-zinc-950 border border-white/5 rounded w-full animate-fadeIn">
                <img src={companyBanner} className="w-full h-16 sm:h-20 object-cover rounded opacity-80" />
              </div>
            )}
          </div>
        </div>

        {/* SECTION 3: TIMELINE METRICS */}
        <div className="space-y-3.5 sm:space-y-4 border border-white/5 p-3 sm:p-4 rounded-xl bg-[#070708]">
          <h3 className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Chronological Timeline Range</h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
            <div className="flex flex-col gap-1 group/input">
              <label className={labelStyle}>
                <span>Start Date <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
                <div className="flex items-center gap-1.5">
                  {isTouched && isStartDateInvalid ? (
                    <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase">missing_start</span>
                  ) : startDate ? (
                    <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase">initialized</span>
                  ) : null}
            ...
            <div className="flex flex-col gap-1 group/input">
              <label className={labelStyle}>
                <span>Terminus Date <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
              ...
              />
              {isTouched && isEndDateInvalid && (
                <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Chronological failure: Terminus date cannot scale before entry milestone.</p>
              )}
            </div>
          </div>
          ...
          <label className="flex items-center gap-2 text-xs sm:text-sm font-bold text-zinc-400 select-none cursor-pointer group pt-0.5">
            ...
            />
            <span className="group-hover:text-white transition-colors">Currently Employed Within This Domain</span>
          </label>
        </div>

        {/* SECTION 4: RESPONSIBILITIES TAGS */}
        <div className="space-y-3.5 sm:space-y-4 border border-white/5 p-3 sm:p-4 rounded-xl bg-[#070708]">
          <h3 className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Execution Milestones & Duties <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></h3>
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  addItem(responsibilityInput, setResponsibilityInput, responsibilities, setResponsibilities);
                }
              }}
            ...
            <button
              type="button"
              onClick={() => addItem(responsibilityInput, setResponsibilityInput, responsibilities, setResponsibilities)}
              disabled={loading}
        ...
        {/* SECTION 5: TECHNOLOGIES TAGS */}
        <div className="space-y-3.5 sm:space-y-4 border border-white/5 p-3 sm:p-4 rounded-xl bg-[#070708]">
          <h3 className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Engineered Architecture Tech Stack <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></h3>
        ...
        <div className="flex flex-col gap-1 group/input">
          <label className={labelStyle}>
            <span>Core Experience Description Summary <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
          </label>
          <textarea
            value={description}
        ...
        {/* FORM OPERATIONAL CONTROL DRAWER */}
