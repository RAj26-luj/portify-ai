...
export default function SettingsForm({
  ...
  // Password Verification Flow States
...
const handleCopyLink = async () => {
  ...
  async function sendPasswordCode() {
    try {
      ...
      const response = await fetch(
        "/api/account/change-password/request-code",
        { method: "POST" }
      );
      ...
      showFeedback("Security verification code dispatched to your registered email address.", "success");
    } catch (error) {
      showFeedback(error instanceof Error ? error.message : "Failed to send code structure.", "error");
    } finally {
  ...
  async function verifyPasswordCode() {
    try {
      ...
      const response = await fetch(
        "/api/account/change-password/verify-code",
        {
          method: "POST",
      ...
      showFeedback("Cryptographic validation successful. Please declare your updated credentials token.", "success");
    } catch (error) {
      showFeedback(error instanceof Error ? error.message : "Security verification failed.", "error");
    } finally {
  ...
  async function handlePasswordChange() {
    try {
      ...
      showFeedback(error instanceof Error ? error.message : "Failed to change system pass-token.", "error");
    } finally {
  ...
  async function handleVisibilityChange(checked: boolean) {
    try {
      ...
      showFeedback(error instanceof Error ? error.message : "Failed to change operational visibility state mapping.", "error");
    } finally {
  ...
  async function sendDeleteCode() {
    try {
      ...
      const response = await fetch(
        "/api/account/delete/request-code",
        { method: "POST" }
      );
      ...
      showFeedback(error instanceof Error ? error.message : "Failed to emit destruction authorization code.", "error");
    } finally {
  ...
  async function verifyDeleteCode() {
    try {
      ...
      const response = await fetch(
        "/api/account/delete/verify-code",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      showFeedback(error instanceof Error ? error.message : "Invalid authorization signature token path.", "error");
    } finally {
  ...
  async function handleDeleteAccount() {
    ...
    try {
      ...
      showFeedback(error instanceof Error ? error.message : "Account drop protocol aborted due to infrastructure exception.", "error");
    } finally {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 text-zinc-300 font-sans max-w-full overflow-x-hidden selection:bg-blue-500/30 selection:text-white select-none">
      ...
      {message && (
        <div className={`p-3 sm:p-3.5 rounded-lg sm:rounded-xl border flex items-start gap-2 sm:gap-3 text-xs animate-fadeIn ${
          messageType === "error" ? "border-red-500/10 bg-red-500/5 text-red-400" :
          messageType === "success" ? "border-emerald-500/10 bg-emerald-500/5 text-emerald-400" :
          "border-blue-500/10 bg-blue-500/5 text-blue-400"
        }`}>
          {messageType === "error" ? <AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" /> :
           messageType === "success" ? <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 shrink-0" /> :
      ...
      <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2">
        ...
        {/* PORTFOLIO VISIBILITY BOARD CARD */}
          ...
          <div className="space-y-3.5 sm:space-y-4">
            <div>
              <h3 className="text-xs sm:text-sm font-bold text-zinc-200 uppercase tracking-widest font-mono flex items-center gap-2">
                {isPublic ? <Eye className="w-3.5 h-3.5 text-blue-400 sm:w-4 sm:h-4" /> : <EyeOff className="w-3.5 h-3.5 text-zinc-500 sm:w-4 sm:h-4" />}
                <span>Portfolio Visibility Canvas</span>
              </h3>
              <p className="text-[10px] sm:text-[11px] text-zinc-500 mt-1 leading-relaxed">
                Control outer ingress routing. New portfolios are completely private drafts by default to protect configuration stages.
              </p>
            </div>
            ...
            <div className="mt-4 sm:mt-5 p-3 sm:p-4 rounded-xl border border-emerald-500/10 bg-emerald-500/[0.02] space-y-3 animate-fadeIn">
              <div className="flex flex-col gap-2.5">
                <div className="min-w-0">
                  <p className="text-xs font-bold text-zinc-300">Your profile path is globally active</p>
                  <p className="text-[10px] sm:text-xs text-zinc-500 font-mono truncate mt-0.5 break-all">{publicPortfolioUrl}</p>
                </div>
        ...
        {/* SECURITY CHANGE CREDENTIALS PASSWORD CARD */}
        <div className="rounded-xl border border-white/10 bg-[#0C0C0E] p-4 sm:p-6 shadow-[0_4px_30px_rgba(0,0,0,0.4)] transition-colors space-y-3.5 sm:space-y-4">
          <div>
            ...
            <p className="text-[10px] sm:text-[11px] text-zinc-500 mt-1 leading-relaxed">
              Step-by-step structural authorization reset. Modifying this credentials layer drops out active sessions.
            </p>
          </div>
          ...
          <div className="space-y-3">
            {!passwordCodeSent && (
              <div className="space-y-2">
                ...
                >
                  {sendingPasswordCode ? <Loader2 size={12} className="animate-spin shrink-0" /> : null}
                  <span>{sendingPasswordCode ? "Requesting Signature..." : "Request Reset Token"}</span>
                </button>
              </div>
            )}
            ...
            {passwordCodeSent && !passwordVerified && (
              <div className="space-y-2.5 animate-fadeIn group/input">
                <label className="text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-widest text-zinc-500 block">
                  Verification Token <span className="text-red-400 font-sans font-bold">*(Required)</span>
                </label>
                <input
                  type="text"
                  maxLength={6}
                ...
                >
                  {verifyingPasswordCode ? <Loader2 size={12} className="animate-spin shrink-0" /> : null}
            ...
            {passwordVerified && (
              <div className="space-y-2.5 animate-fadeIn group/input">
                <label className="text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-widest text-zinc-500 block">
                  New Secure Passphrase <span className="text-red-400 font-sans font-bold">*(Required)</span>
                </label>
                <input
                  type="password"
                ...
                >
                  ...
                  <span>{changingPassword ? "Overwriting Hashes..." : "Commit Passphrase Reset"}</span>
                </button>
              </div>
            )}
          </div>
      ...
      {/* COMPACT SECURE SIGN OUT HOVER DOCK */}
      <div className="rounded-xl border border-white/10 bg-[#0C0C0E] p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3.5 sm:gap-4 shadow-[0_4px_30px_rgba(0,0,0,0.4)]">
        <div className="space-y-0.5">
          ...
          <p className="text-[10px] sm:text-[11px] text-zinc-500 max-w-xl leading-relaxed">
            Instantly drop cryptographically signed token mappings and return back to public template authorization roots.
          </p>
        </div>
      ...
      {/* CRITICAL INFRASTRUCTURE DESTRUCTION DANGER ZONE */}
      <div className="rounded-xl border border-red-500/20 bg-gradient-to-b from-red-950/10 to-transparent p-4 sm:p-6 space-y-3.5 sm:shadow-[0_4px_30px_rgba(0,0,0,0.4)]">
        <div className="space-y-0.5">
          ...
          <p className="text-[10px] sm:text-[11px] text-zinc-500 leading-relaxed max-w-2xl">
            Irreversible destructive zone protocol node. Executing this step drops your custom subdomains, nested resume extraction indices, analytics clusters, and profile sections immediately from the relational database layers.
          </p>
        </div>
        ...
        <div className="pt-0.5">
          ...
          {codeSent && !verified && (
            <div className="space-y-3 max-w-sm animate-fadeIn group/input">
              <label className="text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-widest text-red-400/70 block">
                Purge Authorization Token <span className="text-red-400 font-sans font-bold">*(Required)</span>
              </label>
          ...
          {verified && (
            ...
            >
              {deletingAccount ? <Loader2 size={12} className="animate-spin shrink-0" /> : <Trash2 size={13} />}
              <span>{deletingAccount ? "Purging Entities..." : "Yes, Purge Everything Permanently"}</span>
            </button>
          )}
        </div>
      </div>
