...
export default function SocialLinkForm({ portfolioId, initialData, onSuccess, onClose }: Props) {
    ...
    >
      <div className="bg-[#121214] border border-white/5 rounded-lg p-3 sm:p-4 mb-1">
        ...
        <p className="text-[11px] text-zinc-400 leading-relaxed font-sans">
          Mount routing links pointing outward to your community coordinates (e.g. GitHub, LinkedIn,
          Twitter). Register clean URLs to properly map your outward profile presentation matrix.
        </p>
      </div>
      ...
      <div className="space-y-1 group/input">
          ...
          <div className="flex items-center gap-1.5">
            {isTouched && isPlatformInvalid ? (
              ...
              <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5">
                <CheckCircle2 size={9} /> brand_mapped
              </span>
            ) : null}
            {hasChanges && (
        ...
        {isTouched && isPlatformInvalid && (
          <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">
            ⚠️ Structural parameter missing: Network name mapping identity string required.
          </p>
        )}
        <span className={descriptionStyle}>
          Specify the platform namespace accurately to initialize proper identity markers.
        </span>
      </div>
      ...
      <div className="space-y-1 group/input">
        ...
        />
        <span className={descriptionStyle}>
          Your unique localized handle code on this network layout.
        </span>
      </div>
      ...
      <div className="space-y-1 group/input">
        <label className={labelStyle}>
          <span>
            Absolute Profile URL Link{" "}
            <span className="text-red-400 font-sans font-bold">*(Required)</span>
          </span>
          <div className="flex items-center gap-1.5">
            {isTouched && isUrlInvalid ? (
              <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase flex items-center gap-0.5">
                <AlertCircle size={9} /> broken_vector
              </span>
            ) : url.trim() && !isUrlInvalid ? (
              <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5">
                <CheckCircle2 size={9} /> link_resolved
              </span>
            ) : null}
        ...
        {isTouched && isUrlInvalid && (
          <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">
            ⚠️ Network error: Target address string must match an absolute http/https URI layout
            syntax.
          </p>
        )}
        <span className={descriptionStyle}>
          The structural target hyperlink route vector directly referencing your profile account.
        </span>
      </div>
      ...
      <div className="space-y-3 border border-white/5 p-3 sm:p-4 rounded-xl bg-[#070708]">
        <div>
          ...
          <span className={descriptionStyle}>
            Configure a tool, brand vector, or networking logo layer block.
          </span>
        </div>
        ...
        <div className="flex flex-col gap-1 group/input">
          ...
          <div className={scrapeRecommendationStyle}>
            ...
            <span>
              <strong>Vector Logo Acquisition Protocol:</strong> We highly recommend searching for
              the specific network logo (e.g. GitHub SVG, LinkedIn vector icon) on Google Images,
              Simple Icons, or Wikipedia. Right-click the element asset frame, execute{" "}
              <strong>&quot;Copy Image Address / URL&quot;</strong>, and paste the clean token path
              string directly inside this input block.
            </span>
          </div>
