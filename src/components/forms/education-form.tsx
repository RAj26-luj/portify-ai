...
import { Loader2, Terminal, Calendar, Award, GraduationCap, MapPin, Globe, BookOpen, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function EducationForm({
  ...
  // Evaluates deep delta modifications to conditionally lock down submit triggers
  useEffect(() => {
    ...
    setHasChanges(
      isInstitutionChanged || isDegreeChanged || isFieldChanged || isGradeChanged ||
      isCgpaChanged || isLocationChanged || isImageChanged || isLogoChanged ||
      isDescChanged || isStartChanged || isEndChanged || isCurrentChanged
    );
  }, [institution, degree, fieldOfStudy, grade, cgpa, location, institutionImage, logoUrl, description, startDate, endDate, currentlyStudying, initialData]);
  ...
  const isEndDateInvalid = !!(!currentlyStudying && startDate && endDate && new Date(endDate) < new Date(startDate));

  const isFormInvalid = isInstitutionInvalid || isDegreeInvalid || isFieldInvalid || isStartDateInvalid || isEndDateInvalid;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-3 sm:p-4 select-none animate-fadeIn">
      ...
      >
        ...
        <div className="flex justify-between items-center border-b border-white/5 pb-2.5 sm:pb-3">
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-zinc-500 mt-0.5 font-medium leading-tight font-mono uppercase tracking-wider">
              ...
              {hasChanges && (
                <span className="ml-2 text-[8px] tracking-normal font-mono text-blue-400 bg-blue-500/5 border border-blue-500/10 px-1 py-0.5 rounded uppercase font-bold">unsaved_deltas</span>
              )}
            </p>
          </div>
        ...
        <div className="bg-[#121214] border border-white/5 rounded-lg p-3 text-[11px] text-zinc-400 leading-relaxed font-sans">
          <strong className="text-zinc-200">Education Track Telemetry Node:</strong> Log academic registry details below. Specifying institutional parameters and fields of study cleanly builds high-fidelity data matrices on your public profile sheet.
        </div>
        ...
        <div className="space-y-3.5 sm:space-y-4">
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Educational Institution <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
              <div className="flex items-center gap-1">
                {isTouched && isInstitutionInvalid ? (
                  <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase flex items-center gap-0.5"><AlertCircle size={9} /> missing_registry</span>
                ) : institution.trim() ? (
                  <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5"><CheckCircle2 size={9} /> structured</span>
                ) : null}
            ...
            />
            {isTouched && isInstitutionInvalid && (
              <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Field missing: University configuration identity must be declared.</p>
            )}
            <span className={descriptionStyle}>The structural organization, university, or high school campus body name registry code.</span>
          </div>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Conferred Degree <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
              <div className="flex items-center gap-1">
                {isTouched && isDegreeInvalid ? (
                  <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase flex items-center gap-0.5"><AlertCircle size={9} /> missing_tier</span>
                ) : degree.trim() ? (
            ...
            />
            {isTouched && isDegreeInvalid && (
              <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Field missing: Degree operational mapping cannot remain unpopulated.</p>
            )}
            <span className={descriptionStyle}>The formal structural tier classification title conferred (e.g., B.Tech, M.S., High School).</span>
          </div>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Field of Study <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
              <div className="flex items-center gap-1">
                {isTouched && isFieldInvalid ? (
                  <span className="text-[8px] font-mono text-red-400 bg-red-500/5 px-1 rounded border border-red-500/10 lowercase flex items-center gap-0.5"><AlertCircle size={9} /> missing_focus</span>
                ) : fieldOfStudy.trim() ? (
                  <span className="text-[8px] font-mono text-emerald-400 bg-emerald-500/5 px-1 rounded border border-emerald-500/10 lowercase flex items-center gap-0.5"><CheckCircle2 size={9} /> registered</span>
                ) : null}
              </div>
            </label>
            <input
            ...
            />
            {isTouched && isFieldInvalid && (
              <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Field missing: Specialization sector string vector required.</p>
            )}
            <span className={descriptionStyle}>Your primary specific corporate focus, industry branch, or departmental specialization vector.</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
            <div className="flex flex-col gap-1 group/input">
              <label className={labelStyle}>
                <span>Classification Grade <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
              </label>
              <input
                value={grade}
            ...
            <div className="flex flex-col gap-1 group/input">
              <label className={labelStyle}>
                <span>CGPA / Percentage Score <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
          ...
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Geographic Location <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
        ...
        {/* IMAGE UPLOADS SECTOR */}
        <div className="space-y-3.5 sm:space-y-4 border-t border-white/5 pt-4">
          
          {/* ASSET ACQUISITION PROTOCOL BOX */}
          <div className={scrapeRecommendationStyle}>
            <Globe size={11} className="shrink-0 mt-0.5 text-amber-400" />
            <span>
              <strong>Campus Graphics Advice:</strong> If you prefer dynamic web asset links instead of direct disk uploads, we recommend searching for your university banner or campus emblem on Google Images, right-clicking to select <strong>&quot;Copy Image Address / URL&quot;</strong>, and feeding it to data structures.
            </span>
          </div>

          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Institution Background Cover Image <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
            </label>
            <input
              type="file"
              accept="image/*"
            ...
            {institutionImage && (
              <div className="mt-1.5 p-1 bg-zinc-950 border border-white/5 rounded w-fit animate-fadeIn">
                <img src={institutionImage} alt="Institution preview" className="w-16 h-12 rounded object-cover opacity-80" />
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Institutional Insignia / Logo URL Asset <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
            </label>
            <input
              type="file"
              accept="image/*"
            ...
            {logoUrl && (
              <div className="mt-1.5 p-1 bg-zinc-950 border border-white/5 rounded w-fit animate-fadeIn">
                <img src={logoUrl} alt="Logo preview" className="w-12 h-12 rounded object-contain opacity-80" />
              </div>
            )}
          </div>
        </div>

        {/* CHRONOLOGICAL RANGE TIMELINE METRICS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4 border-t border-white/5 pt-4">
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Timeline Start Date <span className="text-red-400 font-sans font-bold">*(Required)</span></span>
          ...
          <div className="flex flex-col gap-1 group/input">
            <label className={labelStyle}>
              <span>Timeline Terminus Date <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
            ...
            />
            {isTouched && isEndDateInvalid && (
              <p className="text-[10px] font-mono font-medium text-red-400/90 pt-0.5">⚠️ Chronological failure: Terminus date cannot scale before initialization.</p>
            )}
          </div>
        </div>

        {/* CURRENT STATUS FLAG CHECKBOX */}
        <label className="flex items-center gap-2 text-xs sm:text-sm font-bold text-zinc-400 select-none cursor-pointer group pt-0.5">
          ...
          />
          <span className="group-hover:text-white transition-colors">Currently Studying Within This Program Matrix</span>
        </label>

        {/* DETAILS DESCRIPTION FRAMEWORK */}
        <div className="flex flex-col gap-1 border-t border-white/5 pt-4 group/input">
          <label className={labelStyle}>
            <span>Scholastic Activity Details Summary <span className="text-zinc-600 font-sans font-normal lowercase italic">*(Optional)</span></span>
          ...
          />
          <span className={descriptionStyle}>Highly recommended. Detail complex electrical frameworks, algorithmic achievements, or event organization roles (e.g., Nitrutsav coordinator logistics).</span>
        </div>

        {/* FORMS INTERACTION ACTION CONTAINER BUTTON DRAWER */}
