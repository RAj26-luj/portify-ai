...
export default async function Navbar() {
  ...
  // Fresh server-side DB lookups ensure instant reactive badge updates on router.refresh()
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-white/[0.06] select-none">
      <div className="max-w-7xl mx-auto flex justify-between items-center h-16 px-4 sm:px-6 lg:px-8">
        ...
        {/* Left Side: System Logo & Core Navigation Map */}
        <div className="flex items-center gap-3 sm:gap-8 min-w-0">
          ...
          >
            <div className="w-5 h-5 rounded bg-blue-600 flex items-center justify-center font-black text-[10px] text-white shadow-[0_0_12px_rgba(59,130,246,0.4)] shrink-0">P</div>
        ...
        {/* Right Side: Security Context Authentication Actions Deck */}
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          {session?.user ? (
            <>
              {/* Desktop and Tablet authentication status string context mapping */}
              <span className="hidden md:flex items-center gap-2 text-xs font-medium text-zinc-500 font-mono">
                ...
                {/* Modified query strategy to rely directly on live database select rather than stale JWT session token */}
                {portfolio?.isPublic ? (
              ...
              {/* Mobile compact view state visibility flag label mapping badge */}
              {!portfolio?.isPublic && (
              ...
              >
                <LayoutDashboard size={12} className="text-blue-400 shrink-0 sm:w-[13px] sm:h-[13px]" />
                <span>Dashboard</span>
              </Link>
            </>
          ) : (
