...
function handleSkillCategoryServerError(error: any, fallbackMessage: string) {
  ...
  if (
    ...
    return {
      success: false,
      error:
        "Authentication mapping missing. Could not link this skill category to an active portfolio profile.",
    };
  }
  if (
    ...
    return {
      success: false,
      error:
        "The taxonomy management engine is temporarily performing database updates. Please try again.",
    };
  }
...
export async function createSkillCategory(
  ...
  try {
    ...
    if (!resolvedPortfolioId) {
      return {
        success: false,
        error: "Portfolio connection target not found. Unable to create classification group.",
      };
    }
    ...
    if (!data.name) {
      return {
        success: false,
        error: "Skill category label or classification group name parameter cannot be left blank.",
      };
    }
    ...
    return handleSkillCategoryServerError(
      error,
      "Failed to instantiate new skill categorization grid container."
    );
  }
}
...
export async function getSkillCategoryById(categoryId: string) {
  try {
    ...
    return handleSkillCategoryServerError(
      error,
      "Failed to track specific profile core tech category metrics context lines."
    );
  }
}
...
export async function updateSkillCategory(
  ...
  try {
    if (!categoryId) {
      return {
        success: false,
        error: "Missing distinct track classification indices identification key context.",
      };
    }
    ...
    return handleSkillCategoryServerError(
      error,
      "Failed to commit updated nomenclature configurations to skill category properties."
    );
  }
}
...
export async function deleteSkillCategory(categoryId: string) {
  try {
    if (!categoryId) {
      return {
        success: false,
        error: "Identification tracking code missing. Removal pipeline cancelled.",
      };
    }
    ...
    return handleSkillCategoryServerError(
      error,
      "The specified modular tech category index row could not be cleared."
    );
  }
}

export async function reorderSkillCategories(portfolioId: string, categoryIds: string[]) {
  try {
    const resolvedPortfolioId = portfolioId || (await getPortfolioId());
    ...
    if (!resolvedPortfolioId) {
      return {
        success: false,
        error: "Target structural context identity criteria not resolved. Sorting aborted.",
      };
    }
    ...
    if (!categoryIds || categoryIds.length === 0) {
      return {
        success: true,
        message: "Empty sequencing definitions grid list provided. Layout hierarchy unchanged.",
      };
    }
    ...
    return handleSkillCategoryServerError(
      error,
      "Failed to commit customized category grid sequence alignment reorders."
    );
  }
}
