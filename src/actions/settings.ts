...
function handleSettingsServerError(error: any, fallbackMessage: string) {
  ...
  if (
    ...
    return {
      success: false,
      error:
        "Authentication reference token is missing. Could not modify portfolio publication status.",
    };
  }
  if (
    ...
    return {
      success: false,
      error:
        "The deployment engine is temporarily busy running server synchronization. Please toggle status again.",
    };
  }
  ...
  try {
    ...
    if (!portfolioId) {
      return {
        success: false,
        error:
          "Active portfolio profile connection target not found. Visibility adjustments aborted.",
      };
    }
    ...
    return handleSettingsServerError(
      error,
      "Failed to update public deployment configuration metrics."
    );
  }
}
