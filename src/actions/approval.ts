...
import { logUserApproval, logUserBlocked } from "@/lib/audit-log";
...
function handleApprovalServerError(
  ...
  if (
    ...
    return {
      success: false,
      error:
        "State partially updated, but notifications dispatch failed. Review network connectivity.",
    };
  }
...
export async function approveUser(userId: string) {
  try {
    ...
    return handleApprovalServerError(
      error,
      "Failed to complete account approval lifecycle execution flow."
    );
  }
}
...
export async function rejectUser(userId: string, reason?: string) {
  try {
    ...
    return handleApprovalServerError(
      error,
      "Failed to complete account rejection lifecycle execution flow."
    );
  }
}
...
export async function getApprovalRequest(userId: string) {
  try {
    ...
    return handleApprovalServerError(
      error,
      "Failed to retrieve specified context mapping profile relationship verification record."
    );
  }
}
