...
function handleCustomSectionServerError(error: any, fallbackMessage: string) {
  ...
  if (errorMessage.includes("portfolioId required") || errorMessage.includes("portfolioId not found")) {
    return { 
      success: false, 
      error: "Authentication mapping missing. Could not link this custom section to an active portfolio profile." 
    };
  }
  if (errorMessage.includes("Prisma") || errorMessage.includes("database") || errorMessage.includes("Mongo")) {
    return { 
      success: false, 
      error: "The custom layout engine is temporarily locked while performing maintenance. Please try again." 
    };
  }
...
export async function createCustomSection(data: {
  ...
  try {
    ...
    if (!resolvedPortfolioId) {
      return { success: false, error: "Portfolio connection target not found. Unable to create custom section." };
    }
    ...
    return handleCustomSectionServerError(error, "Failed to instantiate a new custom section module layout.");
  }
}
...
export async function updateCustomSection(
  ...
  try {
    if (!id) {
      return { success: false, error: "Missing custom section configuration identity tracker key." };
    }
    ...
    return handleCustomSectionServerError(error, "Unable to save configuration settings to this custom section layout.");
  }
}
...
export async function getCustomSections(portfolioId: string) {
  try {
    ...
    if (!resolvedPortfolioId) {
      return { 
        success: false, 
        error: "Unable to retrieve modular layout components. No portfolio tracking identifier found.",
        data: [] 
      };
    }
...
export async function getCustomSectionById(id: string) {
  try {
    ...
    return handleCustomSectionServerError(error, "Failed to load specified unique section layout elements registry.");
  }
}
...
export async function deleteCustomSection(id: string) {
  try {
    if (!id) {
      return { success: false, error: "Identification key reference missing. Removal pipeline cancelled." };
    }
    ...
    return handleCustomSectionServerError(error, "The selected modular section block could not be successfully cleared.");
  }
}