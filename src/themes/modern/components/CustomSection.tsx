...
export default function CustomSection({ sections = [] }: CustomSectionProps) {
  ...
  const startMobileMarquee = async (fromY: number) => {
    ...
    if (!isDraggingMobile.current && !selectedItemRef.current && isMountedRef.current) {
  ...
  const startDeskMarquee = async (fromX: number) => {
    ...
    if (!isDraggingDesk.current && !selectedItemRef.current && isMountedRef.current) {
      requestAnimationFrame(() => {
        startDeskMarquee(0);
      });
    }
  ...
  return (
    <>
      {activeSections.map((section: any) => {
        return (
          ...
          >
              ...
              <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
                <div className="text-left">
                  ...
                  <h2 className="text-3xl md:text-5xl font-black tracking-tight bg-gradient-to-b from-white via-[#D4D4D8] to-[#71717A] bg-clip-text text-transparent font-sans">
                    {section.title}<span className="text-[#06B6D4]">.</span>
                  </h2>
                </div>
              </div>
            </div>
              ...
              >
                {section.mobileMarqueeItems.map((item: any, idx: number) => (
                  <div
                    ...
                    className="w-full bg-[#111113] border border-[#18181B] active:border-[#6366F1]/50 rounded-xl p-4 flex items-center gap-4 shadow-xl shrink-0 cursor-pointer text-left transition-colors hover:bg-gray-50"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-sm text-white truncate font-sans">{item.title}</h3>
                      {item.subtitle && (
                        <p className="text-[11px] font-mono font-medium text-[#71717A] truncate mt-1 uppercase tracking-wider">{item.subtitle}</p>
                      )}
                    </div>
                  </div>
                ))}
            ...
            {/* ========================================== */}
            {/* 2. DESKTOP VIEW: PREMIUM SAAS SCROLLING MARQUEE */}
            {/* ========================================== */}
            <div className={`hidden md:block relative w-full overflow-hidden py-6 ${section.isScrollable ? "group/marquee cursor-grab active:cursor-grabbing" : ""}`}>
                ...
                {section.marqueeItems.map((item: any, idx: number) => (
                  <div
                    ...
                    className={`bg-[#111113]/80 border border-[#18181B] hover:border-[#6366F1]/30 p-5 rounded-2xl backdrop-blur-xl transition-all duration-300 shadow-[0_20px_40px_rgba(0,0,0,0.4),inset_0_1px_0_0_rgba(255,255,255,0.02)] cursor-pointer hover:-translate-y-1 overflow-hidden group/card text-left ${
                      section.isScrollable ? "w-[330px] sm:w-[390px] shrink-0 inline-block" : "w-full"
                    }`}
                  >
                    ...
                    {item.description && (
                      <p className="text-xs md:text-sm text-[#71717A] font-medium mt-4 whitespace-normal leading-relaxed line-clamp-2 px-1 font-sans">
                        {item.description}
                      </p>
                    )}
                  </div>
      ...
      {/* MODAL REDESIGN: FULL SPEC ARCHITECTURE DASHBOARD OVERLAY */}
      <AnimatePresence>
        {selectedItem && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              <div className="relative h-52 sm:h-72 bg-[#18181B] overflow-hidden border-b border-[#18181B]">
                ...
                <div className="absolute bottom-5 left-5 right-5 sm:bottom-6 sm:left-6 sm:right-6 flex items-center gap-4 text-left">
                  <div className="w-14 h-14 rounded-xl border border-[#18181B] bg-[#0A0A0B] overflow-hidden flex items-center justify-center shrink-0 shadow-2xl p-2">
                    {selectedItem.iconUrl ? (
                      <img src={selectedItem.iconUrl} alt={selectedItem.title} className="w-full h-full object-contain" />
                    ) : (
              ...
              <div className="p-5 sm:p-8 space-y-6">
                {selectedItem.description && (
                  <div className="space-y-2 text-left">
                    <h4 className="text-[11px] font-bold text-[#71717A] tracking-wider uppercase font-mono">Registry Specifications</h4>
