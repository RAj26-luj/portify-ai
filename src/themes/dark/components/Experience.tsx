    ...
    >
      <style jsx global>{`
        @keyframes mission-pulse {
          0%, 100% { transform: scale(1); opacity: 0.5; box-shadow: 0 0 0px rgba(0,255,157,0); }
          50% { transform: scale(1.1); opacity: 1; box-shadow: 0 0 12px rgba(0,255,157,0.5); }
        }
        .cyber-timeline-spine {
      ...
      {/* Cyberpunk Environment Grid Layout */}
      <div className="absolute inset-0 cyber-grid-experience pointer-events-none" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_80%,rgba(124,58,237,0.06),transparent_50%)] pointer-events-none" />
      <div className="absolute top-1/4 right-5 w-80 h-80 bg-[#00E5FF]/4 rounded-full filter blur-[100px] pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 z-10 mb-12 md:mb-24">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-[#00E5FF]/10 pb-6">
          <div className="text-left">
            ...
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-white font-mono uppercase drop-shadow-[0_0_10px_rgba(0,229,255,0.25)]">
              // CAREER_LOG_UPLINK
            </h2>
          </div>
          <div className="hidden lg:flex items-center gap-2 font-mono text-xs text-neutral-500 uppercase tracking-widest">
            ...
            <span>TIMELINE_METRICS: DEPLOYED</span>
          </div>
        </div>
      </div>

      {/* Cyberpunk Timeline Engine */}
      <div className="relative max-w-5xl mx-auto px-4 sm:px-8 lg:px-12 z-10">
        
        {/* Core Architectural Vertical Spine Line */}
        <div className="absolute left-4 sm:left-1/2 top-2 bottom-2 w-[1px] bg-gradient-to-b from-[#00E5FF]/60 via-neutral-800 to-transparent -translate-x-1/2 hidden sm:block cyber-timeline-spine" />
        <div className="absolute left-8 top-2 bottom-2 w-[1px] bg-gradient-to-b from-[#00E5FF]/60 via-neutral-800 to-transparent block sm:hidden cyber-timeline-spine" />
          ...
          {sortedExperiences.map((item: any, idx: number) => {
            ...
            return (
              ...
              >
                ...
                <div className="w-full sm:w-[45%] order-2 sm:text-left flex flex-col">
                  
                  {/* Holographic Mission Block Design */}
                  <div className="w-full bg-[#0B1120] border border-neutral-800 group-hover:border-[#00E5FF]/60 p-5 md:p-6 rounded-none backdrop-blur-xl transition-all duration-300 shadow-2xl group-hover:-translate-y-1 relative overflow-hidden pl-16 sm:pl-6 space-y-4">
                    ...
                    {/* Tech Node Corner Trims */}
                    <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-neutral-700 group-hover:border-[#00E5FF] transition-colors" />
                    <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-neutral-700 group-hover:border-[#7C3AED] transition-colors" />
                    <div className="absolute -right-16 -top-16 w-32 h-32 bg-[#7C3AED]/3 rounded-full blur-2xl pointer-events-none" />

                    {/* Left Structural Frame Accent Line */}
                    <div className={`absolute top-0 bottom-0 w-[2px] bg-gradient-to-b from-[#7C3AED]/40 to-transparent group-hover:from-[#7C3AED] ${isEven ? "sm:right-0 sm:left-auto" : "sm:left-0 sm:right-auto"} hidden sm:block`} />

                    {/* Mobile View Icon Override Placement Frame */}
                    <div className="absolute left-4 top-5 w-9 h-9 border border-neutral-800 bg-[#050816] overflow-hidden flex items-center justify-center sm:hidden shrink-0 shadow-lg mix-blend-luminosity group-hover:mix-blend-normal transition-all">
                      {logo ? (
                        <img src={logo} alt="" className="w-full h-full object-cover filter contrast-125" />
                      ) : (
                    ...
                    <div className="flex flex-col gap-2">
                      <span className="text-[10px] font-mono font-bold text-[#7C3AED] bg-[#7C3AED]/10 border border-[#7C3AED]/30 px-2.5 py-0.5 rounded-sm self-start shadow-[0_0_10px_rgba(124,58,237,0.1)]">
                    ...
                    {/* Metadata Parameter Docks */}
                    {(item.employmentType || item.location) && (
                      <div className="flex flex-wrap gap-2 pt-3.5 border-t border-neutral-900 text-[10px] font-mono justify-start">
                        {item.employmentType && (
                        ...
                        {item.location && (
                          <span className="text-neutral-400 bg-[#050816] border border-neutral-800 px-2.5 py-1 flex items-center gap-1.5">
                ...
                {/* Central Interlocking Axis Center Indicator dot */}
                <div className="absolute left-4 sm:left-1/2 top-6 sm:top-8 w-3.5 h-3.5 rounded-full bg-[#050816] border-2 border-[#00E5FF] z-20 -translate-x-1/2 flex items-center justify-center">
                  ...
                  <span className="absolute inset-0 border border-[#00FF9D] rounded-full opacity-0 group-hover:animate-[mission-pulse_1.5s_ease-in-out_infinite]" />
                </div>

                {/* Desktop Spacer Alignment Column */}
      ...
      {/* IMMERSIVE OPERATIONAL FOCUS MODAL OVERLAY */}
      <AnimatePresence>
        {selectedItem && (
          <motion.div 
          ...
          >
            ...
            >
              {/* Terminal Frame Interface Control Bar */}
              <div className="bg-[#050816] border-b border-neutral-800 px-4 py-2.5 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#00FF9D] animate-pulse" />
                  <span className="text-[10px] font-mono text-neutral-400 uppercase tracking-widest">MISSION_INSPECTOR // OPERATIONAL_RECORD</span>
                </div>
                <button 
                  onClick={() => setSelectedItem(null)}
              ...
              <div className="p-6 sm:p-8 space-y-6 bg-[#0B1120]">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 bg-[#050816] border border-neutral-800 border-l-2 border-l-[#7C3AED]">
                    <div className="text-[9px] font-mono text-neutral-500 uppercase tracking-widest mb-1 flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-[#7C3AED]" /> OPERATIONS_TENURE</div>
                  ...
                  <div className="p-4 bg-[#050816] border border-neutral-800 border-l-2 border-l-[#00E5FF]">
                    <div className="text-[9px] font-mono text-neutral-500 uppercase tracking-widest mb-1 flex items-center gap-1.5"><Layers className="w-3.5 h-3.5 text-[#00E5FF]" /> CONDUIT_TYPE</div>
                    <div className="text-xs font-mono font-bold text-neutral-200 mt-1 truncate uppercase" title={selectedItem.employmentType || "FULL_TIME_TRACK"}>
                      {selectedItem.employmentType || "FULL_TIME_TRACK"}
                    </div>
                  </div>
                  <div className="p-4 bg-[#050816] border border-neutral-800 border-l-2 border-l-[#00FF9D]">
                    <div className="text-[9px] font-mono text-neutral-500 uppercase tracking-widest mb-1 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-[#00FF9D]" /> SECTOR_LOCATION</div>
                    <div className="text-xs font-mono font-bold text-neutral-200 mt-1 uppercase truncate" title={selectedItem.location || "REMOTE_DEPLOYMENT"}>
                      {selectedItem.location || "REMOTE_DEPLOYMENT"}
                    </div>
                  </div>
                </div>
                ...
                {selectedItem.description && (
                  <div className="space-y-2 text-left">
                    <h4 className="text-[10px] font-bold text-neutral-500 tracking-[0.2em] uppercase font-mono flex items-center gap-2">
                      <span className="w-4 h-[1px] bg-neutral-700" /> MISSION_CORE_OBJECTIVE
                    </h4>
                    <p className="text-xs sm:text-sm leading-relaxed text-neutral-300 font-mono whitespace-pre-line bg-[#050816] p-4 rounded-none border border-neutral-800 max-h-[140px] overflow-y-auto scrollbar-none">
                      {selectedItem.description}
                    </p>
                  </div>
                )}

                {/* Responsibilities Deliverables Bullet Stack */}
                {selectedItem.responsibilities?.length > 0 && (
                  <div className="space-y-2 text-left">
                    <h4 className="text-[10px] font-bold text-neutral-500 tracking-[0.2em] uppercase font-mono flex items-center gap-2">
                      <span className="w-4 h-[1px] bg-neutral-700" /> SYSTEM_DELIVERABLES_LOG
                    </h4>
                    <ul className="space-y-2 text-xs sm:text-sm font-mono text-neutral-300 max-h-[180px] overflow-y-auto pr-2 scrollbar-none">
                      {selectedItem.responsibilities.map((r: string, i: number) => (
                        <li key={i} className="flex items-start gap-2.5 leading-relaxed">
                          <span className="w-1.5 h-1.5 rounded-none bg-[#00FF9D] mt-2 shrink-0 shadow-[0_0_8px_#00FF9D]" />
                          <span>{r}</span>
                        </li>
                      ))}
                    </ul>
                ...
                {/* Engine Technologies Blueprint Layer */}
                {selectedItem.technologies?.length > 0 && (
                  <div className="space-y-2 text-left">
                    <h4 className="text-[10px] font-bold text-neutral-500 tracking-[0.2em] uppercase font-mono flex items-center gap-2">
                      <span className="w-4 h-[1px] bg-neutral-700" /> SYSTEM_CAPABILITIES_MAPPED
                    </h4>
                ...
                {/* Secure Gateway Footing Console Navigation */}
                <div className="pt-4 border-t border-neutral-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-mono text-xs text-neutral-500">
                  <div onClick={(e) => e.stopPropagation()} className="w-full sm:w-auto">
                    {selectedItem.companyWebsite && (
                      ...
                      >
                        LAUNCH_CORPORATE_GATEWAY <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                    )}
                  </div>
                  <span className="flex items-center gap-2 justify-center sm:justify-start text-[10px] tracking-widest uppercase">
                    <Radio className="w-3.5 h-3.5 text-[#00FF9D] animate-pulse" /> {identityName.toUpperCase()} // MISSION_STREAM_SYNCHRONIZED
                  </span>
                </div>
              </div>
            </motion.div>
